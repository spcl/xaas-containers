...
def determine_vectorization(architecture, flags):
    ...
    vectorization_flags = {
        "x86_64": [
            "avx-512", "avx512f", "avx512dq", "avx512cd", "avx512bw", "avx512vl",
            "avx2", "avx", "sse4_2", "sse4_1", "ssse3", "sse3", "sse2", "sse"
        ],
        "aarch64": [
            "asimd", "sve"
        ],
...
def get_accelerators():
    ...
    return {
        "Accelerators": dict(accelerators) if accelerators else ["No accelerators detected"],
...
def get_parallel_libraries():
            ...
            for root, _, files in os.walk(path):
                for file in files:
                    for lib_name in mpi_libraries:
                        # Match libmpi.so, libmpi.so.12, libmpi.so.12.0.3, etc.
                        if re.fullmatch(f'{lib_name}(\\.\\d+)*', file):
                            full_path = os.path.join(root, file)
                            version = file.split(".so.")[-1] if ".so." in file else "unknown"
                            lower_path = path.lower()
                            if "intel-oneapi-mpi" in lower_path:
                                mpi_impl = "Intel MPI"
                            elif "openmpi" in lower_path:
                                mpi_impl = "OpenMPI"
                            elif "mpich" in lower_path:
                                mpi_impl = "MPICH"
                            elif "cray" in lower_path or "cray-mpich" in lower_path:
                                mpi_impl = "Cray MPI"
                            elif any(keyword in lower_path for keyword in ["rom-openmpi", "roc-ompi", "ompi"]):
    ...
    if not found_libraries:
        found_libraries["No MPI Found"] = {"None": "No MPI shared libraries detected in LD_LIBRARY_PATH"}
    ...
    if cuda_home:
        ...
        if os.path.exists(cufft_path):
            for lib_name in ["libcufft", "libcufftw"]:
                lib_files = [f for f in os.listdir(cufft_path) if f.startswith(lib_name) and ".so." in f]
                if lib_files:
                    # Find fully versioned shared library (e.g., libcufft.so.11.0.2.54)
                    full_versioned_lib = sorted(lib_files, key=lambda x: list(map(int, re.findall(r'\d+', x))))[-1]
        ...
        if lib_paths:
            if fft_lib_name == "MKL":
                ...
                if filename_version:
                    ...
                    try:
                        version_output = subprocess.check_output(f"strings {lib_paths[0]} | grep -Ei 'version|release'", shell=True, text=True)
...
def get_opencl_version():
    ...
    try:
        output = subprocess.check_output("strings /lib64/libOpenCL.so.1 | grep -i 'OpenCL'", shell=True, text=True)
...
def get_gpu_backends():
    ...
    if hip_libs:
        ...
        gpu_backends["HIP"]["version"] = check_version("hipcc --version", r"HIP version: (\d+\.\d+\.\d+)")
    ...
    for backend in ld_versions:
        if gpu_backends[backend]["version"] == "Version not found" or gpu_backends[backend]["version"] is None:
